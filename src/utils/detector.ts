...
const getProjectDetails = (): ProjectDetails => {
  ...
  const reactExists = packageJsonExists && fs.existsSync(path.join(currentDir, "node_modules", "react")) && !fs.existsSync(path.join(currentDir, "node_modules", "next"));
  const nextJsExists = packageJsonExists && fs.existsSync(path.join(currentDir, "node_modules", "next"));
  const nextAppSrcRouterExists = fs.existsSync(path.join(currentDir, "src", "app"));
  const nextAppRouterExists = fs.existsSync(path.join(currentDir, "app"));
  const pageSrcRouterExists = fs.existsSync(path.join(currentDir, "src", "pages"));
...
const writeToFile = (filePath: string, data: string, fileName: string): void => {
  fs.writeFile(filePath, data, (err) => {
    if (err) {
      console.error("🚨 Error writing file:");
    } else {
      console.log(`✔ Created ${fileName} at ${filePath}`.green);
    }
  });
};
